...
import { renderSpreadHTML, autoShrinkText } from "../src/lib/editor/card-to-html-v2";
...
async function main(): Promise<void> {
  ...
  if (ids.length === 0) {
    console.log("Usage: npx tsx scripts/screenshot-v2.ts TI04");
    console.log("       npx tsx scripts/screenshot-v2.ts --all");
    return;
  }
  ...
  for (const id of ids) {
    ...
    try {
      ...
      // Render card at actual mm dimensions, use deviceScaleFactor for quality
      const mmToPx = 3.7795; // 1mm = 3.7795px at 96dpi
      const cardW = Math.round(config.spreadWidthMm * mmToPx);
      const cardH = Math.round(config.spreadHeightMm * mmToPx);
      await page.setViewport({ width: cardW, height: cardH, deviceScaleFactor: 3 });

      const path = `${OUT_DIR}/${id}.png`;
      // Clip to exactly the card area
      await page.screenshot({ path, clip: { x: 0, y: 0, width: cardW, height: cardH } });
      console.log(`${id}: screenshot → ${path}`);
      await page.close();
    } catch (e: unknown) {
