...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const fullHTML = await renderSpreadHTML(state);

    if (!hasBackPage) {
      // Single-page template — return as-is
    ...
    const frontHTML = await renderSpreadHTML(frontState);
    const backHTML = await renderSpreadHTML(backState);

    // Combine into a two-page spread
...
<body>
  <div>
    ...
    <div class="page">
      <iframe srcdoc="${escapeAttr(frontHTML)}" width="${spreadWidthMm * 2.5}" height="${spreadHeightMm * 2.5}" sandbox="allow-same-origin"></iframe>
    </div>
  </div>
  <div>
    <div class="page-label">Rückseite</div>
    <div class="page">
      <iframe srcdoc="${escapeAttr(backHTML)}" width="${spreadWidthMm * 2.5}" height="${spreadHeightMm * 2.5}" sandbox="allow-same-origin"></iframe>
    </div>
  </div>
</body>
</html>`;
